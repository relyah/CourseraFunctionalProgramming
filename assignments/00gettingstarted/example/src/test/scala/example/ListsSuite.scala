...
class ListsSuite extends FunSuite {
  ...
  test("sum of negative elements") {
    assert(sum(List(-10, 10, 20)) === 20)
  }

  test("sum of with repeated elements") {
    assert(sum(List(10, 10, 20)) === 40)
  }
  ...
  test("max of a list with negative elements") {
    assert(max(List(3, -3, 0)) === 3)
  }
  
    test("max of a list with elements that are all 0") {
