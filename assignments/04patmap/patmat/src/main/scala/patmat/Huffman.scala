...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def times0(sub: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = sub match {
      case Nil => acc
      case x :: xs => times0(xs.filter(a => a != x), List.concat(acc, List((x, 1 + xs.count(b => b == x)))))
    }
    times0(chars, List())
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def make0(acc: List[Leaf], sub: List[(Char, Int)]): List[Leaf] = sub match {
      case Nil => acc
      case x :: xs => make0(List.concat(acc, List(Leaf(x._1, x._2))), xs)
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    {
      def decode0(subtree: CodeTree, bits0: List[Bit], acc: List[Char]): List[Char] =
        (subtree, bits0) match {
          case (Leaf(c, _), h :: t) => decode0(tree, bits0, acc :+ c)
          case (Leaf(c, _), _) => acc :+ c
          case (Fork(l, _, _, _), 0 :: t) => decode0(l, t, acc)
          case (Fork(_, r, _, _), 1 :: t) => decode0(r, t, acc)
...
object Main extends App {
  ...
  def enc2 = quickEncode(t1)(string2Chars("abd"))
  println(enc2);
  println(decode(t1, enc2))
